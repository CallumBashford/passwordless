 ...
 * @param {Boolean} [options.overrideErrors] If set, passwordless will no longer wrap
 * user verification layer error messages (default: false)
...
Passwordless.prototype.requestToken = function(getUserID, options) {
	...
	return function(req, res, next) {
		...
		getUserID(user, delivery, function(uidError, uid) {
			if(uidError) {
				if(options.overrideErrors) {
					// Quit wrapping my error messages
					return next(uidError);
				}
